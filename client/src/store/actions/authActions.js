...
const meetingSecurityStart = () => {
    return {
        type: MEETING_SECURITY_START
    };
};

const meetingSecuritySuccess = () => {
    return {
        type: MEETING_SECURITY_SUCCESS
    };
};
const meetingSecurityFail = (err) => {
    return {
        type: MEETING_SECURITY_FAIL,
        err
    };
};
...
export const updateTimezone = (updatedZone) => {
    return (dispatch, getState, { getFirebase, getFirestore }) => {
        dispatch(timezoneStart())
        const firestore = getFirestore();
        firestore.collection("users").doc(updatedZone.userId).update({
            timezone: updatedZone.selectedTimezone,
        })
            .then(() => {
                dispatch(timezoneSuccess());
                updatedZone.push('/profile');
            }).catch((err) => {
                dispatch(timezoneFail(err.message))
            })
    }
}
...
export const updateMeetingSettings = (meeting) => {
    return (dispatch, getState, { getFirebase, getFirestore }) => {
        dispatch(meetingSecurityStart())
        const firestore = getFirestore();
        firestore.collection("users").doc(meeting.userId).update({
            lockMeeting: meeting.lockMeeting,
            waitingRoom: meeting.waitingRoom,
            participantsShareScreen: meeting.participantsShareScreen,
            participantsMuteUnmute: meeting.participantsMuteUnmute,
            participantsVideoOnOff: meeting.participantsVideoOnOff
        })
            .then(() => {
                dispatch(meetingSecuritySuccess());
                meeting.push('/profile');
            }).catch((err) => {
                dispatch(meetingSecurityFail(err.message))
            })
    }
}
